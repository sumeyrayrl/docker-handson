...
- Display the `ubuntu` os info on the container (`VERSION="24.04.2 LTS (Jammy Jellyfish)"`) and note that the `latest` tag is showing release `24.04` as in the Docker Hub. Then exit the container.
...
- Download earlier version (`24.04`) of `ubuntu` image, which is tagged as `24.04` on Docker Hub and explain image list.
...
```bash
docker run -d --name welcome2 -p 8080:80 clarusway/flask-app:2.0
docker ps
```
